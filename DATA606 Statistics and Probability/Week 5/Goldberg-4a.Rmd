...
It has a mean of 1550, unimodal, very slightly right skewed
...
sd(area)
```
It has a mean of 1501, so very close to the population. Unimodal. It has a shorter standard deviation, so its a slimmer curve. 
...
summary(samp2)
```
samp2 has a mean of 1520, a little further from the population mean than samp1. If we went to sample size 100 then to n=1000, we would expect to have less and less deviation from the population mean. 
...
There are 5000 elements. The sampling distribution is very close to a normal distribution, unimodal. With 50,000 sample means, it would come even closer to approximating the normal distribution. 
...
sample_means_small
```
Each element represents the mean of one of the 100 samples
...
The sample size shouldn't affect where the center lies, but larger sample sizes have higher peaked curves, as each of the sample means is more likely to be closer to the actual mean and therefore less spread out. 
* * *
## On your own
...
par(mfrow = c(1, 1))
hist(sample_means50)
#Shape is unimodal, no clear skew, mean is around 180000, non-outlier boundaries are around 140000 to 220000, so about SD is about 2k

summary(sample_means50)
#Sample distribution mean is `180700`

summary(price)
#Actual population mean is `180800`
```
...
hist(sample_means150, bin = 30)
#Unimodal, no clear skew, mean close to `180000`, SD around `2000`
...
#Shape and distribution of both n=150 and n=50 is very similar. Based on histogram, I would guess mean price of `1800000`

summary(sample_means150)
summary(sample_means50)
#Both are 100 off: n=150 is `180900`, n=50 is `180700`
```
...
#n=150 has a very slightly larger spread with current sample calculations, but its negligable. 

#presumably you would want less spread, and therefore smaller variance and standard deviations allowing for more accuracy. 
```
