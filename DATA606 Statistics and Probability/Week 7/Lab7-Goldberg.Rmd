...
```{r}
#Using a scatter plot to gauge relationship by eye
plot(mlb11$at_bats, mlb11$runs)
```
It does look linear and positive, and I would feel comfortable using a linear model to use at_bats as a prediction of runs
...
The relationship is positive, appears to be linear, and has a fairly strong correllation. There are a few potential outliers consisting of instances where a batter scored more runs than normal on fewer at bats, and a few outliers with the opposite conditions. 
...
My smallest sum of squares: 154599.3
...
\]
The slope tells us it is a positive relationship, and for every one homerun, estimation of runs increases by about 2
...
If I was the manager and I was only shown the line, with no formula or data, I would guess roughly 525 runs. 

```{r}
#This would have a residual of: 
...
No clear pattern, so there's no obvious curvature in the model and it should be linear. 
...
The histogram appears fairly normal as does the normal probability plot
*Constant variability*:

8.  Based on the plot in (1), does the constant variability condition appear to 
    be met?
Yes, the variability around the least squares line appears to be constant
* * *
...
```{r}
summary(ba) #r^2 = .6561
.6561-.3729 #batting average appears to predict runs better than at bats, by roughly .28
```
...
Batting average better predicts runs, with an r^2 of .6561
```{r}
summary(lm(mlb11$runs ~ mlb11$bat_avg)) #r^2 = .6561
plot(mlb11$runs ~ mlb11$bat_avg)
#summary(lm(mlb11$runs ~ mlb11$stolen_bases)) #r^2 = 0.002914
#summary(lm(mlb11$runs ~ mlb11$strikeouts)) #r^2 = 0.1694
#summary(lm(mlb11$runs ~ mlb11$wins)) #r^2 = 0.361
#summary(lm(mlb11$runs ~ mlb11$homeruns)) #r^2 = 0.6266
#summary(lm(mlb11$runs ~ mlb11$at_bat)) #r^2 = 0.3729
```
...
```{r}
#summary(lm(mlb11$runs ~ mlb11$new_onbase)) #r^2 = 0.8491
#summary(lm(mlb11$runs ~ mlb11$new_slug)) #r^2 = 0.8969
no <- lm(mlb11$runs ~ mlb11$new_obs)
summary(no) #r^2 = 0.9349
plot(mlb11$runs ~ mlb11$new_obs)
```
All three variables outperform batting average, with obs the strongest at .9348. This does make sense. Batting average does not take walks into account, which are good at explaining the batter's ability to control the strike zone and pitch recognition. Slugging also adds value because it captures a players power, or their ability to bring runners home. 


-   Check the model diagnostics for the regression model with the variable you 
    decided was the best predictor for runs.
```{r}
plot(no$residuals~mlb11$new_obs)
abline(h = 0, lty = 3) #no obvious patterns

hist(no$residuals) #seems normal
qqnorm(no$residuals)
qqline(no$residuals) #seems normal
```
