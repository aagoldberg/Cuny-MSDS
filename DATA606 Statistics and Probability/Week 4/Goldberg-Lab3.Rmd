---
title: 'AGoldberg Lab 3: The normal distribution'
output:
  pdf_document: default
  html_document:
    css: C:\Users\Andrew\Documents\R\win-library\3.1\IS606\labs\Lab3\lab.css
    highlight: pygments
    theme: cerulean
---
...
setwd("C:\\Users\\Andrew\\Documents\\R\\win-library\\3.1\\IS606\\labs\\Lab3")
...
library("ggplot2")
qplot(hgt, data = mdims, geom = "histogram", binwidth = 3)
qplot(hgt, data = fdims, geom = "histogram", binwidth = 3)
#They both appear unimodal and symmetric, and probably normally distributed. The men's appears to have a wider curve (larger SD).
```
...
A: Visually, the density histogram appears to approximate the normal distribution. 
...
```{r sim-norm-construct, eval=TRUE}
...
A: The sim points follow the line slightly more closely than the actual data, but it has simiarly errant data near the tails. 
...
A: The female heights data is slightly more erratic near the apex than the sims, but close enough that it looks nearly normal. 


5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.

```{r hist-fweight, eval=TRUE}
#Create vars for avg and sd of women's weight
fwgtmean <- mean(fdims$wgt)
fwgtsd   <- sd(fdims$wgt)

#Plot density histogram to take a first look
hist(fdims$wgt, probability = TRUE)
x <- 40:110
y <- dnorm(x = x, mean = fwgtmean, sd = fwgtsd)
lines(x = x, y = y, col = "blue")

#Construct normal probability plot for a second look
qqnorm(fdims$wgt)
qqline(fdims$wgt)

#Create some sim data and compare (although appears duplicitous with qqnormism?)
fwsim_norm <- rnorm(n = length(fdims$wgt), mean = fwgtmean, sd = fwgtsd)
qqnorm(fwsim_norm)
qqline(fwsim_norm)

#Compare data to several sims
qqnormsim(fdims$wgt)
```

A: Without a lot of experience eyeballing normal distributions, I am slightly less comfortable saying women's weight follows a normal distribution, since it's points veer off pretty quickly at the tails. But I'd still bet on it being close enough. 
...
Q: What percentage of women are taller than 175 cm? (5'9")
```{r fheight-over175, eval=TRUE}
#Theoretical = .061
...
Q: What percentage of women weigh more than 59 kg? (130lb)
```{r fweight-over59, eval=TRUE}
#Theoretical = .57
...
On height above 175cm, the difference was .077 - .061 = .016
On weight above 59kg, the difference was .57 - .50 = .7

So height had closer agreement, and also started further along the tail
* * *
...
A: My guess is that there are large steps between each density bin. It makes sense with age, which is measured pretty discretely.I assume that is happening with elbow diameter as well, but it's hard to eyeball. 
...
```{r probplot-fkneedi, eval=TRUE}
#Create vars for avg and sd of women's knee diameter
...
#Plot density histogram to double check
