...
A major assumption is that the sample was randomly selected. While I'm sure they put a lot of resources in the work, there are many geopolitical boundaries where they were probably not able to sample from, such as North Korea.
...
Each row of table 6 is a country observation

Each row of atheism is an individual level observation
...
Has to be less than 10% of the population -- true
Data has to come from a random process -- that is standard survey methodology
There are at least ten success and ten failures -- true
...
```{r fr-pk-atheism-ci, tidy = FALSE}
fr12 <- subset(atheism, nationality == "France" & year == "2012")
pk12 <- subset(atheism, nationality == "Pakistan" & year == "2012")
summary(fr12)
summary(pk12)
#I believe that both sample less than 10% of their population, I assume it's random
#In both cases there are at least 10 successes and 10 failures

inference(fr12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
inference(pk12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```
France's confidence interval for .29 atheist is (.2657, .3089) or a MoE of .0243
Pakistan's ci for .20 atheist is (.0147, .0252), MoE of .1853

France has more atheists. And, because france has a larger sample size and more atheists, the margin of error is much smaller. 
...
Margin of error peaks when proportion is at .5, because .5*.5 is highest possible numerator
...
summary(p_hats)
#mean, center is .10
#Shape is unimodal, with no noticable skew

sd(p_hats)
#spread hsa a .0092 SD
```
...
for(i in 1:5000){
  samp <- sample(c("atheist", "non_atheist"), n4, replace = TRUE, prob = c(p, 1-p))
  p_hats1[i] <- sum(samp == "atheist")/n4
}
...
for(i in 1:5000){
  samp <- sample(c("atheist", "non_atheist"), n4, replace = TRUE, prob = c(p2, 1-p2))
  p_hats3[i] <- sum(samp == "atheist")/n4
}
par(mfrow = c(2,2))
hist(p_hats, main = "p = 0.1, n = 1040", xlim = c(0.06, 0.15))
hist(p_hats1, main = "p = 0.1, n = 400", xlim = c(0.04, 0.17)) #more spread, lower sample
hist(p_hats2, main = "p = 0.2, n = 1040", xlim = c(0.15, 0.24)) #more spread at n=1040, p=.2 than p=.1
hist(p_hats3, main = "p = 0.2, n = 400", xlim = c(0.12, 0.27)) #more spread @p=.02, n=400 than at n=1040

#overall, more spread with higher p, lower n
par(mfrow = c(1, 1))
```
...
This is an odd question. As a pollster, we usually write "the margin of error is XXX at the 95% confidence interval. The margin of error on subsamples is greater". So the margin of error does change with smaller subsamples, and that should be assumed by the reader. Or maybe I'm reading the question incorrectly. 
...
#ho: there is no difference in atheism rates in Spain between 2005 and 2012
#ha: there is a difference in atheism rates in Spain between 2005 and 2012
...
#2005 mean = .1003, (.083, .1177)
#2012 mean = .09, (.0734, .1065)
#so lots of overlap, and we therefore fail to reject the null
```
...
#ho: there is no difference in atheism rates in the US between 2005 and 2012
#ha: there is a difference in atheism rates in the US between 2005 and 2012
...
#2005 mean = .01, (.0038, .0161)
#2012 mean = .0499, (.0364, .0634)
#No overlap, so we reject the null in favor of the hypothesis: atheism rates in the US have probably changed
```
...
Out of 37 countries with data, we would expect about 5% to have a type 1 error, which would be 1.85 counties, so about 1 or 2
