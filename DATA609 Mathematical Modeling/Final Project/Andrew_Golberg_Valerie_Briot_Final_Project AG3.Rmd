---
title: "CUNY 609 - Final project"
author: "Andrew Golberg, Valerie Briot"
date: "May 21, 2017"
output:
...
(First, the question states that Methy mercury decays at 50% "if no further mercury is ingest during that time". Since the average adult person will be ingesting a fish per day, the question provides flawed information, but ok...)

###Formula formation
Half-life of Mercury (M) over 70 days:
$$\ M(t) = M_{o}e^{-kt}$$
...
knitr::kable(life_merc[60:71,] , format="markdown")
```
...
```
**Mercury Accumulation after 30 Days**  
...
knitr::kable(max_30[28:30,101:103], format = "markdown")
...
```{r}
knitr::kable(max_30[1:4,101:103], format = "markdown")
plot((1:nrow(max_30)), max_30[,100], xlab = "Day", ylab = "Grams of mercury left in body", main = "Mercury in body daily")
```

**Mercury Accumulation on 3rd Day**  
```{r}
knitr::kable(max_30[1:4,90:97], format = "markdown")
...
*** 
#The Spread of a Contagious Disease (problem 5 page 499)
...
a) Since this is an autonomous differential equation, perform a qualitative graphical analysis as discussed in Section 11.4. 
	(i)  Plot dN/dt versus N. Find and label all rest points (equilibrium points).  
	
  (ii)  Estimate the value where the rate of change of the disease is the fastest. Justify your answer.  
...
b)	Obtain a slope field plot of this differential equation. Briefly analyze the slope field plot. 
    Compare it to your qualitative plot in part (a) above.
    
c)	Solve this differential equation using the separation of variables technique.  
    Plot the solution over the interval of time from [0, 10].
    How does your actual solution compare to the qualitative graphical solution?
    
d)	Compute the time t when N is changing the fastest using the initial condition N(0) = 2.  
    Compare your answer to your qualitative estimate in part a(iii) above.
    
e)	Use Euler's Method with step sizes of h=1 and then h=0.1 to approximate the solution to the differential equation for N(0.5) and N(5).  
    Find the relative error at these values. Obtain graphical plots of the numerical solution.  
    How do they compare to the plots of the actual solution?  
    
f)	Repeat part(e)using either the Improved Euler's Method or the Runge-Kutta Method. 
...
We will first plot dN/dt versus N, for some value of N. Equilibrium (or rest point) are all the values of N for which dN/dt = 0.  We will find these by setting the equation to Zero.  

$\frac { dN }{ dt } =.25N(10-N)=0$  

This will hold true when N=0 or 10-N = 0, hence we have possible values of N that satisfy the equation to be N=0 or N=10.  

We will first construct a phase line for the equation by ploting the equilibrium value on the y axis and analysis the intervals where $\frac { dN }{ dt }$ and $\frac { { d }^{ 2 }N }{ { (dt) }^{ 2 } }$ are positive and negative.  

We will first derive $\frac { { d }^{ 2 }N }{ { (dt) }^{ 2 } }$ by differentiating N' with respect to t using implicit differentiation.  
...
We will now analyse the sign (positive/negative) for $\frac { dN }{ dt }$ and $\frac { { d }^{ 2 }N }{ { (dt) }^{ 2 } }$   
...
![phase line](C:\Users\vbrio\Documents\Cuny\DATA_609\Final Project\final_figure1.png)

**Analysis**  

N=0 and N=10 are equilibrium points. 

The solution curve below line N=0 woud fall away from line N=0, however, since N>0, this is not part of solution curve.
...
###ii) Fastest rate of change of the disease  
...
###iii) From this information we will now plot N versus t  

See figure 2: Solution Curves N(t)  

![Solution Curves N(t)](C:\Users\vbrio\Documents\Cuny\DATA_609\Final Project\final_figure2.jpg)


###iv) Stability of equilibrium point(s):  

Once the solution curve has a value near N=10, it tends steadily towards this value, N=10 is a stable equilibrium.

However for N=0, is a trivial solutioln, since we would have no population and no growth.  

***

##b. Slop Field Analysis

We need to express dt in function of slope and dN.  



```{r Slope Field, warning=FALSE}
...
##c. Sloving Differential equation
...
We will consider the initial values for N similar to a(i); N(0)=2, N(0)=7, and N(0)=14  we will substitute $quad { t }_{ 0 }$ and $quad { N }_{ 0 }$ by the above value in the appropriate equation.  


```{r functions plotting, warning=FALSE}
...
The solutions curves, hand drawn in a(iii) are very similar to the graphs obtained by plotting the equations.  

***

##d. Compute the time t when N is changing the fastest using the initial condition N(0) = 2  

Since dN/dt represent the rate of change of N, in order to find at what time t, the rate is the fastest, we need to find the maximum for dN/dt.  
To do this we will look at what value of t render $\frac { { d }^{ 2 }N }{ { (dt) }^{ 2 } } =0$  

From a(i) we know that:   
$\frac { { d }^{ 2 }N }{ { (dt) }^{ 2 } } =\frac { 1 }{ 8 } N(10-N)(5-N),\quad for\quad 0<N<10$  
...
Which is a solution.  We now need to determine whether it is a maximum.  we will calculate for t slightly before and slightly after for dN/dt,  
we will use the function fun_N2 previously coded.  

```{r fastest change}

t_solution <- 2/5*(log(4))
...
dndt <- function(t){
  return(0.25*fun_N2(t)*(10-fun_N2(t)))
}

dndt1 <- dndt(t1)
dndt2 <- dndt(t2)
dndt_sol <- dndt(t_solution)


```

for t1 < t_solution, we have for dN/dt = `r dndt1` and for t2 > t_solution we have for dN/dt = `r dndt2` and at t_solution we have `r dndt_sol`.  

Hence we have a maximum.  The value of t that correspond to the fastest change for N is `r t_solution`.  

Which correspond to the understanding we had from the graphical analysis done in a(ii).  

***  

##e. Euler Method to approximate differential equation  

Given an Ordinary Differential equation of the form: $\frac { dy }{ dx } =g(x,y)$,  

the initial value and step method for approximation for a Ordinary Differential equation can be given by the general formula:  

${ y }_{ i+1 }\quad =\quad { y }_{ i }+\phi h$,  
where $\phi$ is the slope and h is the step, $\Delta x$.    

For the Euler method, the slope is $\frac { dy }{ dx }$, ie $g(x,y)\quad at\quad ({ x }_{ i },\quad { y }_{ i })$  
...
To calculate the relative error, we will calculate the actual value of function and then take difference (estimate - actual) 
...
```{r euler case1 table, results='asis'}
knitr::kable(out1)
```

### case2: Initial value: N(0.5) and h=0.1:  
...
```{r euler case2 table, results='asis'}
knitr::kable(out2[1:10,])
```

### case3: Initial value: N(5) and h=1:  
...
```{r euler case3 table, results='asis'}
knitr::kable(out3)
```

### case4: Initial value: N(5) and h=0.1:  
...
```{r euler case4 table, results='asis'}
...
For a small value of h, (small increment of t), the Euler approximations compare well with the actual curves.  However for larger value of h, h=1, the approximation is not able to handle, the handle the approach to N=10.  The approximation is oscillating around this value without seemingly able to converge.  

We will now plot the relative errors for each data set and keep to the same color scheme for easier identification.    

```{r euler relative errors}
...
The values of errors far away from Error = 0, are when h=1, for h=0.1, the approximation gets better as N approaches 10.  

***  

##d. Runge-Kutta 4th order Method to approximate differential equation  


In this method, the slope.h is define as: 

$\phi \cdot h=\frac { { K }_{ 1 }+2{ K }_{ 2 }+2{ K }_{ 3 }+{ K }_{ 4 } }{ 6 } \quad where\quad { K }_{ 1 },\quad { { K }_{ 2 } },\quad { K }_{ 3 },\quad and\quad { K }_{ 4 }\quad are\quad as\quad follows:$  
...
We will use the package deSolve to perform the calculation.  
...
```{r RK4 case1 table, results='asis'}
...
```{r RK4 case2 table, results='asis'}
...
```{r RK4 case3 table, results='asis'}
...
```{r RK4 case4 table, results='asis'}
knitr::kable(out4_rk4[1:10,])
```

We will now plot our estimated N value in function of t.  
...
Rundle-Kutta 4th Order approximation of the solution curves is far superior to Euler method.  Even for a large h, the curves produced are close to actual although showing segment.  For small value of h, the curves produced by approximation are very close to actual.

We will now plot the relative errors for each data set and keep to the same color scheme for easier identification.    

```{r RK4 relative errors}
...
When h=0.1, the approximation to the curve is a very nice fit with relative error of 0. Even for h=1, (gold and aquamarine) points, the approximation is much better than for Euler method.  
