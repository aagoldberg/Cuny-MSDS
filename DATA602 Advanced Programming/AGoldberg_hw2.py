...
def sortbyprice(l, sortl):
    priceD = {}
    priceConv = {'High': 3, 'Med':2, 'Low':1}
    priceL = []
    brandL = []

    for eval in l:
        priceL.append(priceConv[eval.Price])
        priceD[priceConv[eval.Price]] = eval.Brand
    if sortl == 'asc':
        priceL.sort()
    else:
        priceL.sort(reverse=True)
    for pL in priceL:
        brandL.append(priceD[pL])
    return brandL
...
#   otherwise false
def searchforsafety(l, searchl):
    safetyL = []

    for eval in l:
        safetyL.append(eval.SafetyRating)
    if searchl in safetyL:
        return True
    else:
        return False
